...
function Home() {
    ...
    const getLeaderboard = async () => {
        try {
            ...
            const freezeDateTimeStamp = new Date(freezeObj.freezeDate.getTime() - freezeObj.freezeDate.getTimezoneOffset() * 60000);
            console.log(freezeDateTimeStamp);
            let { data, error } = await supabase.from("foc_points").select("*, foc_user(*), foc_group(*), foc_game(*)").order("created_at", { ascending: false });
            ...
            const groupedArray = Object.entries(groupedData).map(([key, value]) => ({
                ...
                max: (value as { point: number }[]).reduce((max, obj) => (max.point > obj.point ? max : obj)).point,
                min: (value as { point: number }[]).reduce((min, obj) => (min.point < obj.point ? min : obj)).point,
            }));
            ...
            for (const group of groups) {
                ...
                // let group = groups.
                leaderboardDict[groups.filter((e) => e.id == group.id)[0].name as string] = currentScore;
            }
            ...
            for (const [game_id, game] of Object.entries(gameData)) {
                ...
                for (const group of game) {
                    ...
                    leaderboardDict[groups.filter((e) => e.id == group.group)[0].name as string] += point;
            ...
            const result = Object.entries(leaderboardDict).map(([groupName, score]) => ({
    ...
    useEffect(() => {
        const channel = supabase
            ...
            .on("postgres_changes", { event: "UPDATE", schema: "public", table: "foc_state" }, (payload) => {
    ...
    return (
        <main className="min-h-[100dvh] bg-white flex flex-col space-y-6 items-center justify-around w-full max-w-sm mx-auto">
            <div className="flex flex-col pt-12">
                ...
                <h1 className="text-3xl text-center text-purple-900 tracking-wide font-bold">BLINK IN TIME</h1>
            </div>

            <img src="/leaderboard/delorean.svg" alt="machine" className="max-w-sm w-full animate-float px-12" />
            ...
            {freeze && (
                <div className="w-full px-4">
                    <Alert className="w-full">
                        ...
                        <AlertDescription>The leaderboard is currently frozen.</AlertDescription>
                    </Alert>
                </div>
            )}
              ...
              <h1 className="px-6 py-1 bg-[#B08D57] text-sm text-[#FFD700]">
                ...
                <div className="flex items-center justify-center w-full max-w-sm pb-12 px-4">
                    <Table className="w-full">
                        <TableCaption>{isLoading ? "Loading Leaderboard..." : "Current standing for SOC FOC 24'"}</TableCaption>
                        <TableHeader>
                            <TableRow>
                                <TableHead className="w-[24px] text-black">Rank</TableHead>
                                <TableHead className="text-black px-1">Group Name</TableHead>
                                <TableHead className="text-center text-black">Total Points</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>{!isLoading && (leaderboard ?? []).map((leader, index) => <LeaderboardRow key={index} {...leader} index={index} />)}</TableBody>
                    </Table>
                </div>
            </div>
        </main>
...
const LeaderboardRow = ({ group_id, group_name, total_points, index }: Leaderboard & { index: number }) => {
    ...
    return (
        <TableRow>
            ...
            <TableCell
                className={cn(["px-1", rank === 1 && "text-[#FFD700] font-bold", rank === 2 && "text-[#C0C0C0] font-bold", rank === 3 && "text-[#B8860B] font-bold"])}
            >
